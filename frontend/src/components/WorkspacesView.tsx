    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="animate-pulse space-y-6">
          <div className="h-8 bg-[#EFECE6] w-1/4 rounded-xs"></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="h-64 bg-[#EFECE6] rounded-sm"></div>
            <div className="md:col-span-2 h-96 bg-[#EFECE6] rounded-sm"></div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 mb-8 border-b border-[#E7E2D8]">
        <div>
          ...
          <p className="text-sm text-[#524E48] font-sans mt-0.5">
        ...
        <button
          ...
          className="inline-flex items-center space-x-2 px-4 py-2.5 bg-[#1A1A1A] hover:bg-[#C85A17] text-[#FAF9F5] text-xs font-mono rounded-sm transition-colors cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-4 space-y-3">
          <h2 className="text-xs font-mono uppercase tracking-widest text-[#706B63] font-bold mb-3">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-sm border transition-all cursor-pointer ${
                    isActive
                      ? 'bg-[#FAF9F5] border-2 border-[#1A1A1A] shadow-[3px_3px_0px_0px_#1A1A1A]'
                      : 'bg-[#F4F1EA] border-[#E7E2D8] hover:border-[#1A1A1A]'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-serif font-bold text-sm text-[#1A1A1A] truncate pr-2">
                      {ws.name}
                    </span>
                    <span className="text-[10px] font-mono bg-[#EFECE6] border border-[#E7E2D8] text-[#524E48] px-2 py-0.5 rounded-xs shrink-0 font-bold">
                      {ws.itemCount} items
                    </span>
                  </div>

                  {ws.description && (
                    <p className="text-xs text-[#524E48] line-clamp-2 mb-2 font-sans">
                      {ws.description}
                    </p>
                  )}

                  <div className="flex items-center justify-between text-[10px] font-mono text-[#706B63] pt-2 border-t border-[#E7E2D8]/60">
        ...
        <div className="md:col-span-8 space-y-6">
          {activeWorkspace ? (
            <div className="bg-[#FAF9F5] border-2 border-[#1A1A1A] rounded-sm p-6 sm:p-8 shadow-[4px_4px_0px_0px_#1A1A1A]">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 mb-6 border-b border-[#E7E2D8]">
                <div>
                  <h2 className="font-serif font-bold text-2xl text-[#1A1A1A]">
                    {activeWorkspace.name}
                  </h2>
                  <p className="text-xs text-[#524E48] font-sans mt-0.5">
                    {activeWorkspace.description || 'Saved references and citations.'}
                  </p>
                </div>

                <div className="text-xs font-mono text-[#706B63]">
              ...
              {activeWorkspace.items && activeWorkspace.items.length > 0 ? (
                <div className="space-y-4">
                  {activeWorkspace.items.map((item) => (
                    <div
                      key={item.id}
                      className="bg-[#F4F1EA] border border-[#E7E2D8] p-4 rounded-sm transition-all"
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        ...
                        <div className="flex items-center space-x-1">
                          <button
                            ...
                            className="px-2 py-1 bg-[#FAF9F5] hover:bg-[#1A1A1A] hover:text-[#FAF9F5] text-[#1A1A1A] border border-[#E7E2D8] text-[10px] font-mono rounded-xs transition-colors cursor-pointer"
                          ...
                          <button
                            ...
                            className="p-1 text-[#706B63] hover:text-rose-700 hover:bg-rose-50 rounded-xs transition-colors cursor-pointer"
                      ...
                      <h3 className="font-serif font-bold text-base text-[#1A1A1A] hover:text-[#C85A17] transition-colors mb-1">
                        <a
                          href={item.url}
                          target="_blank"
                ...
                <div className="py-12 text-center border-2 border-dashed border-[#E7E2D8] rounded-sm">
                  <BookmarkCheck className="w-8 h-8 text-[#9C9588] mx-auto mb-3" />
                  <h3 className="font-serif font-semibold text-base text-[#1A1A1A] mb-1">
                    No Saved References Yet
                  </h3>
                  <p className="text-xs text-[#706B63] max-w-sm mx-auto mb-4 font-sans">
            ...
            <div className="p-8 text-center text-[#706B63] font-mono text-xs">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 bg-[#1A1A1A]/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[#FAF9F5] border-2 border-[#1A1A1A] rounded-sm p-6 sm:p-8 max-w-md w-full shadow-[8px_8px_0px_0px_#1A1A1A] animate-in fade-in zoom-in duration-150">
            ...
            <p className="text-xs text-[#706B63] font-mono mb-6">
            ...
            <form onSubmit={handleCreateWorkspace} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#FAF9F5] border border-[#E7E2D8] focus:border-[#C85A17] p-2.5 text-sm text-[#1A1A1A] rounded-sm focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-[#FAF9F5] border border-[#E7E2D8] focus:border-[#C85A17] p-2.5 text-xs text-[#1A1A1A] rounded-sm focus:outline-none resize-none"
                />
              </div>
